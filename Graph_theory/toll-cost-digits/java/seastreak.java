...
public class Solution {
    ...
    public static void main(String[] args) {
        GraphFindAllPaths<Integer> graphStructure;
        graphStructure = new GraphFindAllPaths<Integer>();
        ...
        for(int a0 = 0; a0 < e; a0++){
            ...
            int y = in.nextInt();
            int r = in.nextInt();
            graphStructure.addNode(x);
            graphStructure.addNode(y);
            graphStructure.addEdge(x, y, r);
            graphStructure.addEdge(y, x, 1000 - r);
        }
        
        FindAllPaths<Integer> findAllPaths = new FindAllPaths<Integer>(graphStructure);
        
        //System.out.println(graphStructure.getWeight(1, 2));
        //System.out.println(findAllPaths.getAllPaths(2, 3));
        
        
        Map<Integer, Map<Pair, Integer>> totalMap = new HashMap<Integer, Map<Pair, Integer>>();
        
        for(int x : graphStructure.graph.keySet()) {
            for(int y : graphStructure.graph.keySet()) {
                if(x == y) {
                    continue;
                }
                List<List<Integer>> pathsList = findAllPaths.getAllPaths(x, y);
                for(List<Integer> path : pathsList) {
                    Pair<Integer, Integer> pair = new Pair<Integer, Integer>(path.get(0), path.get(path.size() - 1));
                    int pathWeight = findPathWeight(path, graphStructure);
                    int endingDigit = findEndingDigit(pathWeight);
                    /*
                    if(totalMap.containsKey(endingDigit)) {
                         Map<Pair, Integer> pairIntegerMap = totalMap.get(endingDigit);    
                    }
                    */
                    ...
                    if(totalMap.containsKey(endingDigit)) {
                        Map<Pair, Integer> pairIntegerMap = totalMap.get(endingDigit);
                        if(!pairIntegerMap.containsKey(pair)) {
                            pairIntegerMap.put(pair, pathWeight);
                            totalMap.put(endingDigit, pairIntegerMap);
                        }
                        
                    } else {
                        Map<Pair, Integer> pairIntegerMap = new HashMap<Pair, Integer>(); 
                        pairIntegerMap.put(pair, pathWeight);
                        totalMap.put(endingDigit, pairIntegerMap);
                    }
        ...
        for(int i=0; i<10; i++) {
            if(totalMap.get(i) == null) {
                ...
                System.out.println(totalMap.get(i).size());    
            }
    ...
    private static int findPathWeight(List<Integer> path, GraphFindAllPaths<Integer> graphStructure) {
        int weight = 0;
        /*
        if(path.size() == 2) {
            weight = weight + graphStructure.getWeight(path.get(i),  path.get(i+1))
        }
        */
    ...
    private static int findEndingDigit(int x) {
        return x%10;
    }
...
private static class GraphFindAllPaths<T> implements Iterable<T> {
    ...
    public int getWeight(T source, T destination) {
        return graph.get(source).get(destination).intValue();
    }
}
 ...
 */
private static class FindAllPaths<T> {
