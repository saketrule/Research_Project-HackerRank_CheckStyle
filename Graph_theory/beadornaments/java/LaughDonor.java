...
public class Solution {
    ...
    static BigInteger [] cumfreq = new BigInteger[1024];
    ...
    public static BigInteger getWays (int mask) {
        ...
        for (int mask1 = 1; mask1 < mask; mask1 ++) {
            ...
            ans = ans.add(getWays(mask1).multiply(getWays(mask2).multiply(cumfreq[mask1].multiply(cumfreq[mask2]))));
        }
    ...
    public static void solve () {
        ...
        for (int i = 0; i < (1 << n); i ++) {
            cumfreq[i] = BigInteger.ZERO;
            count[i] = 0;
            for (int j = 0; j < n; j ++) {
                if (((i >> j) & 1) > 0) {
                    cumfreq[i] = cumfreq[i].add(freqs[j]);
                    count[i] ++;
                }
            }
        }
