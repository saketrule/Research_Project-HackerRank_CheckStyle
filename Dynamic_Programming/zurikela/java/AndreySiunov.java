...
public class Solution {
    ...
    private static final int SZ = 100013;
    ...
    private int mergeSet(int v) {
        while (pr[v] >= 0) v = pr[v];
        Pii t = next(v);
        clear(v);
        return Math.max(t.key, t.value);
    }

    private Pii next(int v) {
        int r1 = a[v];
        int r2 = 0;
        for (Integer nv : g[v]) {
            Pii t = next(nv);
            r1 += t.value;
            r2 += Math.max(t.key, t.value);
        }
        return new Pii(r1, r2);
    }
